...
class WCActionsListener():
    ...
    def listen(self, source, action):
        ...
        if action == 'adds':            
            df = df[mask_minus_one]['token'] + '+'
        elif action == 'dels':
            df = df[df['action'] == 'out']['token'] + '-'
        elif action == 'reins':
            df = df[df['action'] == 'in'].loc[~mask_minus_one]['token'] + '*'
        else:
            df = pd.concat([
                (df[mask_minus_one]['token'] + '+'),
                (df[df['action'] == 'out']['token'] + '-'),
                (df[df['action'] == 'in'].loc[~mask_minus_one]['token'] + '*')
            ])
